...
export default function QueryControlsPanel({ activeWorkflow, catalog, dslValid, parameters, project, projectId, projects, stopping, submitting, workflowInstanceId, workflowState, onLogs, onParameters, onRun, onShowParameters, onStop, onValidity }: QueryControlsPanelProps) {
  return <section className="h-full min-h-0 min-w-0"><div className="h-full overflow-y-auto">
    ...
    <div className="space-y-5 p-5">
      ...
      <QueryField label="回溯周期" value={parameters.lookback} onChange={(lookback) => onParameters({ ...parameters, lookback })} />
      <QueryCodesField codes={parameters.codes} projects={projects} onChange={(codes) => onParameters({ ...parameters, codes })} />
