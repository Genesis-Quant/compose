...
export default function WorkflowPanel({ onTotalChange }: { onTotalChange?: (total: number) => void }) {
...
function WorkflowTableState({ children }: { children: React.ReactNode }) { return <TableRow><TableCell colSpan={8}><div className="flex min-h-40 items-center justify-center gap-2 text-sm text-muted-foreground">{children}</div></TableCell></TableRow>; }
