...
import { findNodeAtLocation, getLocation, getNodeValue, parse, parseTree, type Node, type ParseError } from "jsonc-parser";
...
export default function DslEditor({ catalog, modelPath, onChange, onValidityChange, readOnly = false, value }: DslEditorProps) {
  ...
  function change(nextSource = "") {
    ...
    const parsed = parse(nextSource, errors, { allowTrailingComma: false, disallowComments: true }) as unknown;
    const valid = errors.length === 0 && isDslDocument(parsed);
    onValidityChange?.(valid);
    if (!valid) return;
    currentDocument.current = parsed;
    onChange(parsed);
  }
  ...
  return <div className="dsl-editor flex h-full min-h-72 flex-col overflow-hidden rounded-md border border-border"><div className="min-h-0 flex-1"><Editor
    ...
    options={{
      ...
      suggest: { preview: false, selectionMode: "never", showSnippets: true, showStatusBar: true },
...
function configureJsonLanguage() {
...
function isDslDocument(value: unknown): value is DslDocument {
