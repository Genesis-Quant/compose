...
type QueryProjectTableProps = { loading: boolean; projects: QueryProject[]; onDelete: (project: QueryProject) => void; onOpen: (project: QueryProject) => void };

export default function QueryProjectTable({ loading, projects, onDelete, onOpen }: QueryProjectTableProps) {
  return <Card className="overflow-hidden py-0 shadow-sm"><CardContent className="p-0"><Table className="min-w-[760px] table-fixed"><TableHeader><TableRow><TableHead className="w-[38%] px-5">名称</TableHead><TableHead className="w-40 px-4">状态</TableHead><TableHead className="w-32 px-3">Workflow ID</TableHead><TableHead className="w-48 px-3">更新时间</TableHead><TableHead className="w-16 px-3 text-right">操作</TableHead></TableRow></TableHeader><TableBody>{projects.map((project) => <QueryProjectRow key={project.id} project={project} onOpen={() => onOpen(project)} onDelete={() => onDelete(project)} />)}{loading ? <ProjectTableState colSpan={5}><Loader2 className="animate-spin" />正在加载...</ProjectTableState> : null}{!loading && !projects.length ? <ProjectTableState colSpan={5}>暂无查询项目</ProjectTableState> : null}</TableBody></Table></CardContent></Card>;
}

function QueryProjectRow({ onDelete, onOpen, project }: { onDelete: () => void; onOpen: () => void; project: QueryProject }) {
