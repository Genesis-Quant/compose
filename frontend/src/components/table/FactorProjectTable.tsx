...
import type { FactorProject } from "@/types/factor";

type FactorProjectTableProps = { loading: boolean; projects: FactorProject[]; onDelete: (project: FactorProject) => void; onOpen: (project: FactorProject) => void };

export default function FactorProjectTable({ loading, projects, onDelete, onOpen }: FactorProjectTableProps) {
  return <Card className="overflow-hidden py-0 shadow-sm"><CardContent className="p-0"><Table className="min-w-[1120px] table-fixed"><TableHeader><TableRow><TableHead className="w-[260px] px-5">名称</TableHead><TableHead className="w-28 px-4">最新版本</TableHead><TableHead className="w-28 px-3 text-right">IC 均值</TableHead><TableHead className="w-28 px-3 text-right">RankIC 均值</TableHead><TableHead className="w-24 px-3 text-right">ICIR</TableHead><TableHead className="w-28 px-3 text-right">多空收益</TableHead><TableHead className="w-24 px-3 text-right">多空夏普</TableHead><TableHead className="w-40 px-3">更新时间</TableHead><TableHead className="w-16 px-3 text-right">操作</TableHead></TableRow></TableHeader><TableBody>{projects.map((project) => <FactorProjectRow key={project.id} project={project} onOpen={() => onOpen(project)} onDelete={() => onDelete(project)} />)}{loading ? <ProjectTableState colSpan={9}><Loader2 className="animate-spin" />正在加载...</ProjectTableState> : null}{!loading && !projects.length ? <ProjectTableState colSpan={9}>暂无研究项目</ProjectTableState> : null}</TableBody></Table></CardContent></Card>;
}

function FactorProjectRow({ onDelete, onOpen, project }: { onDelete: () => void; onOpen: () => void; project: FactorProject }) {
