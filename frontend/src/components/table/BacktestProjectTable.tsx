...
import type { BacktestProject } from "@/types/backtest";

type BacktestProjectTableProps = { loading: boolean; projects: BacktestProject[]; onDelete: (project: BacktestProject) => void; onOpen: (project: BacktestProject) => void };

export default function BacktestProjectTable({ loading, projects, onDelete, onOpen }: BacktestProjectTableProps) {
  return <Card className="overflow-hidden py-0 shadow-sm"><CardContent className="p-0"><Table className="min-w-[1180px] table-fixed"><TableHeader><TableRow><TableHead className="w-[260px] px-5">名称</TableHead><TableHead className="w-28 px-4">最新版本</TableHead><TableHead className="w-28 px-3 text-right">累计收益</TableHead><TableHead className="w-28 px-3 text-right">年化收益</TableHead><TableHead className="w-28 px-3 text-right">夏普比率</TableHead><TableHead className="w-28 px-3 text-right">年化波动</TableHead><TableHead className="w-28 px-3 text-right">最大回撤</TableHead><TableHead className="w-24 px-3 text-right">日胜率</TableHead><TableHead className="w-40 px-3">更新时间</TableHead><TableHead className="w-16 px-3 text-right">操作</TableHead></TableRow></TableHeader><TableBody>{projects.map((project) => <BacktestProjectRow key={project.id} project={project} onOpen={() => onOpen(project)} onDelete={() => onDelete(project)} />)}{loading ? <ProjectTableState colSpan={10}><Loader2 className="animate-spin" />正在加载...</ProjectTableState> : null}{!loading && !projects.length ? <ProjectTableState colSpan={10}><Activity className="size-4" />暂无回测项目</ProjectTableState> : null}</TableBody></Table></CardContent></Card>;
}

function BacktestProjectRow({ onDelete, onOpen, project }: { onDelete: () => void; onOpen: () => void; project: BacktestProject }) {
