  ...
  return <div className="min-h-0 flex-1 space-y-5 overflow-y-auto pr-1">
    ...
    {kind === "factor" && pair
      ...
      <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-2">
        <ResultColumn accent="border-l-sky-500">{pair.left ? <Suspense fallback={<ReportLoading />}><FactorAnalysisReport chartRanges={factorRanges} factor={pair.left} key={`${left.task_id}:${pair.left}`} parameters={left.parameters as FactorAnalysisParameters} taskId={left.task_id} onChartRanges={setLeftFactorRanges} onMetrics={ignoreMetrics} /></Suspense> : <MissingFactor />}</ResultColumn>
        <ResultColumn accent="border-l-amber-500">{pair.right ? <Suspense fallback={<ReportLoading />}><FactorAnalysisReport chartRanges={factorRanges} factor={pair.right} key={`${right.task_id}:${pair.right}`} parameters={right.parameters as FactorAnalysisParameters} taskId={right.task_id} onChartRanges={setRightFactorRanges} onMetrics={ignoreMetrics} /></Suspense> : <MissingFactor />}</ResultColumn>
      </div>
      </>
      : null}
    {kind === "backtest"
      ...
      <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-2">
        <ResultColumn accent="border-l-sky-500"><Suspense fallback={<ReportLoading />}><BacktestReport activeTab={activeTab} annualTradingDays={(left.parameters as BacktestParameters).annual_trading_days} chartRanges={backtestRanges} riskFreeRate={(left.parameters as BacktestParameters).risk_free_rate} showTabs={false} taskId={left.task_id} onActiveTabChange={setActiveTab} onChartRanges={setLeftBacktestRanges} onSummary={ignoreMetrics} /></Suspense></ResultColumn>
        <ResultColumn accent="border-l-amber-500"><Suspense fallback={<ReportLoading />}><BacktestReport activeTab={activeTab} annualTradingDays={(right.parameters as BacktestParameters).annual_trading_days} chartRanges={backtestRanges} riskFreeRate={(right.parameters as BacktestParameters).risk_free_rate} showTabs={false} taskId={right.task_id} onActiveTabChange={setActiveTab} onChartRanges={setRightBacktestRanges} onSummary={ignoreMetrics} /></Suspense></ResultColumn>
      </div>
      </>
      : null}
  </div>;
