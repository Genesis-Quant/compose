...
export default function AnalysisWorkspace({ backTo, children, sidebar, sidebarLabel = "分析参数" }: AnalysisWorkspaceProps) {
  ...
  return <div className={cn(
    "grid min-h-[calc(100dvh-4rem)] grid-cols-1 xl:transition-[grid-template-columns] xl:duration-300 xl:ease-out",
    ...
    <aside className="relative flex min-w-0 flex-col border-b bg-muted/15 xl:sticky xl:top-16 xl:h-[calc(100dvh-4rem)] xl:self-start xl:border-b-0 xl:border-r">
      <div className="relative min-h-0 flex-1">
        ...
        <div className={cn("pointer-events-none absolute inset-0 hidden flex-col items-center gap-3 bg-muted/15 pt-10 text-muted-foreground transition-opacity duration-200", collapsed && "xl:flex")}>
      ...
      <div className={cn("relative z-20 flex shrink-0 items-center border-t p-3", collapsed ? "xl:gap-0" : "gap-2")}>
        <div className={cn("min-w-0 overflow-hidden transition-opacity duration-150", collapsed && "xl:pointer-events-none xl:w-0 xl:opacity-0")}>
