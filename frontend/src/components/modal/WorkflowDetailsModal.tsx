...
import { terminalStates, type WorkflowListItem } from "@/types/workflow";
...
export default function WorkflowDetailsModal({ now, onOpenChange, open, workflow }: { now: number; onOpenChange: (open: boolean) => void; open: boolean; workflow: WorkflowListItem | null }) {
  const duration = workflow ? resolveDurationSeconds(workflow.duration_seconds, workflow.started_at, workflow.finished_at, !terminalStates.has(workflow.state), now) : null;
...
function ParameterTabs({ payload }: { payload: WorkflowListItem["payload"] }) {
