    ...
    <DialogContent className="flex max-h-[88vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-[min(980px,calc(100vw-2rem))]">
      ...
      {workflow && <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        <section className="grid gap-px bg-border md:grid-cols-3">
          ...
          <InformationGroup title="执行信息" rows={[["应用", applicationNames[workflow.application]], ["开始时间", formatDate(workflow.started_at)], ["完成时间", formatDate(workflow.finished_at)], ["最后同步", formatDate(workflow.last_synced_at)]]} />
          <InformationGroup title="记录时间" rows={[["创建时间", formatDate(workflow.created_at)], ["更新时间", formatDate(workflow.updated_at)], ["Task 查询", workflow.tasks_error ?? "正常"]]} />
        </section>
        <Separator />
...
function formatDuration(seconds: number | null) { if (seconds === null) return "—"; if (seconds < 60) return `${seconds.toFixed(1)} 秒`; return `${Math.floor(seconds / 60)} 分 ${Math.round(seconds % 60)} 秒`; }
